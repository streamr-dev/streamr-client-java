...
class KeyStorageUtilSpec extends Specification {
    void "hasKey() returns true iff there is a GroupKeyHistory for the stream"() {
        GroupKey key = new GroupKey("group-key")
        when:
        KeyStorage util = KeyStorageUtil.getKeyStorage(["streamId": key], true)
        then:
    ...
    void "getLatestKey() returns null when there is no GroupKeyHistory for the stream"() {
        when:
        KeyStorage util = KeyStorageUtil.getKeyStorage(new HashMap<String, GroupKey>(), true)
        then:
